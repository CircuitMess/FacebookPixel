...
{
  "displayName": "Facebook Pixel by CircuitMess",
  "description": "This is an unofficial Google Tag Manager template for the Facebook Pixel. Originally developed by Simo Ahava and JabJab.",
  "securityGroups": [],
  "categories": [
    "ADVERTISING",
    "ANALYTICS"
  ...
  "version": 1,
  "brand": {
    "displayName": "CircuitMess",
    "id": "github.com_circuitmess",
...
[
  ...
  },
  {
    ...
    "checkboxText": "Ecommerce dataLayer Integration",
    "simpleValueType": true,
    "help": "If you check this, then the Facebook pixel will populate \u003cstrong\u003eEvent Name\u003c/strong\u003e and \u003cstrong\u003eObject Properties\u003c/strong\u003e automatically from the last \u003ca href\u003d\"https://developers.google.com/analytics/devguides/collection/ga4/ecommerce?client_type\u003dgtm\"\u003eecommerce\u003c/a\u003e object pushed into the dataLayer array."
  },
  {
    "type": "RADIO",
    "name": "eventName",
  ...
  {
    "type": "TEXT",
    "name": "eecDefaultCurrency",
    "displayName": "Default currency if not present in the datalayer",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "enhancedEcommerce",
    ...
    "help": "The Ecommerce integration checks for the presence of ecommerce.currency or ecommerce.items[0].currency. If neither is found, this currency will be used. If this fields if left blank, USD will be used as default.",
    "textAsList": true
  },
  {
    "type": "SELECT",
...
if (data.enhancedEcommerce) {
  ...
  if (!ecommerce.items || getType(ecommerce.items) !== 'array') return fail('Facebook pixel: Most recently pushed "ecommerce" object did not have a valid "items" array.');

  if (ecommerce.items.length === 0) return fail('Facebook pixel: Most recently pushed "ecommerce" object does not include product data in the "items" array.');
  ...
  eecObjectProps = {
    ...
    currency: ecommerce.currency || ecommerce.items[0].currency || data.eecDefaultCurrency || 'USD'
  };
  if (['InitiateCheckout', 'Purchase'].indexOf(eventName) > -1) eecObjectProps.num_items = ecommerce.items.reduce((acc,cur) => {
...
- name: Enhanced Ecommerce integration fails with invalid action
  code: "mockData.enhancedEcommerce = true;\n\nmock('copyFromDataLayer', key => {\n\
    ...
    \ \"event\") return \"view_item\";\n});\n\n// Call runCode to run the template's\
    \ code.\nrunCode(mockData);\n\n// Verify that the tag finished successfully.\n\
    assertApi('logToConsole').wasCalledWith('Facebook pixel: Most recently pushed\
    \ \"ecommerce\" object did not have a valid \"items\" array.');\nassertApi('gtmOnFailure').wasCalled();\n\
    assertApi('gtmOnSuccess').wasNotCalled();"
